...
const login = async (payload: TLogin) => {
  ...
  if (user?.isGoogleLogin) {
    throw new AppError(
      ...
      'This user created by login with google. please try to login with google',
    );
  }
...
const googleLogin = async (payload: any) => {
  const isUserHave: IUser | null = await User.isUserExist(payload?.email);
  let jwtPayload;
  try {
    if (!isUserHave) {
      ...
      jwtPayload = {
        userId: isUserHave?._id?.toString() as string,
